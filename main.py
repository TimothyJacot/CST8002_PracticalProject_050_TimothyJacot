...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            try:
                year_input = input("Enter year to filter:")
                year = int(year_input) if year_input else None

                site_id = input("Enter site ID to filter:")
                site_id = site_id if site_id else None

                min_count_input = input("Enter minimum count to filter:")
                min_count = int(min_count_input) if min_count_input else None

                filtered_records = service.filter_records(year=year, site_id=site_id, min_count=min_count)
                print(f"\nFiltered records found: {len(filtered_records)}\n")
